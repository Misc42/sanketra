...
REPO_URL="https://github.com/Misc42/sanketra.git"
INSTALL_DIR="${INSTALL_DIR:-$HOME/sanketra}"
BRANCH="master"
...
install_deps_linux() {
    if all_linux_deps_present; then
        info "All dependencies already present — skipping apt (no sudo needed)"
        return 0
    fi
    detect_pkg_manager
...
check_linux_port() {
    ...
    local pid
    pid=$(ss -ltnHp 2>/dev/null | grep ':5000 ' | grep -oE 'pid=[0-9]+' | head -1 | cut -d= -f2)
    [[ -z "$pid" ]] && return 0

    local cmdline=""
    [[ -r "/proc/$pid/cmdline" ]] && cmdline=$(tr '\0' ' ' < "/proc/$pid/cmdline")

    if echo "$cmdline" | grep -qE "(sanketra|mic_on_term).*server_async"; then
        warn "Port 5000 occupied by previous Sanketra server (PID $pid) — replacing"
